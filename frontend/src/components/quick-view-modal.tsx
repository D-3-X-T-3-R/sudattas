...
function QuickViewActions({
  ...
  return (
    <div className="flex flex-col p-4 md:p-6 md:pr-8">
      ...
      <div className="mt-6">
        <div className="text-sm font-semibold text-[var(--color-ink)]">Size <button type="button" className="font-normal text-[var(--color-muted)] underline">Size Chart</button></div>
        <div className="mt-2 flex flex-wrap gap-2">
          {SIZES.map((size) => (
            <button key={size} type="button" onClick={() => setSelectedSize(size)} className={cn("min-w-[2.5rem] rounded border px-3 py-2 text-sm font-medium transition-colors", selectedSize === size ? "border-[var(--color-ink)] bg-[var(--color-ink)] text-white" : "border-[var(--color-line)] bg-white text-[var(--color-ink)] hover:border-[var(--color-ink)]")}>{size}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 flex items-center gap-2">
          <button type="button" onClick={() => setQuantity((q) => Math.max(1, q - 1))} aria-label="Decrease quantity" className="flex h-10 w-10 items-center justify-center rounded border border-[var(--color-line)] text-lg font-medium hover:bg-[var(--color-line)]/20">-</button>
          <span className="min-w-[2rem] text-center font-sans text-sm font-medium">{quantity}</span>
          <button type="button" onClick={() => setQuantity((q) => q + 1)} aria-label="Increase quantity" className="flex h-10 w-10 items-center justify-center rounded border border-[var(--color-line)] text-lg font-medium hover:bg-[var(--color-line)]/20">+</button>
        </div>
      </div>

      <Button onClick={() => onAddToCart(product, quantity, selectedSize)} className="mt-6 w-full rounded-full bg-[var(--color-accent-gold)] py-6 text-base font-semibold text-[var(--color-ink)] hover:bg-[var(--color-accent-gold)]/90">ADD TO BAG</Button>
      <div className="mt-4 flex items-center gap-2 text-sm text-[var(--color-muted)]"><Truck className="h-4 w-4 shrink-0" /><span>Estimated Delivery by: Wednesday, Mar 16</span></div>
      ...
      <div className="mt-8">
        ...
        <Accordion title="Delivery & Payment"><p>Order is made to order. Dispatch in 7-10 working days; transit 4-5 days. Cash on Delivery available.</p></Accordion>
        <Accordion title="Returns & Exchange"><ul className="list-disc space-y-1 pl-5"><li>3 days easy return & exchange</li><li>Free pickup for returns & exchanges</li><li>Returns/exchanges allowed only for unused products with tags</li></ul></Accordion>
      ...
      <DialogContent title="" showClose onPointerDownOutside={onClose} onEscapeKeyDown={onClose} className="max-w-5xl overflow-hidden p-0" contentClassName="p-0">
        <div className="max-h-[90vh] overflow-y-auto">
          <div className="grid gap-8 md:grid-cols-2">
            ...
            <QuickViewActions product={product} wished={wished} onToggleWish={onToggleWish} onAddToCart={onAddToCart} />
          </div>
        </div>
      </DialogContent>
    </Dialog>
